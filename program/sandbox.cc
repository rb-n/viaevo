...
// Sandbox::Execute also forces this build before fork() so the very first build
...
ExecutionResult Sandbox::Execute(const ElfImage &image, ExecuteMode mode) const {
  ...
  if (pid == 0)
    RunElfProcess(image); // [[noreturn]]: execs or _exit()s in the child.

  result.ptrace_stops = MonitorElfProcess(pid, image, mode, &result);
  return result;
}

int Sandbox::MonitorElfProcess(pid_t elf_pid, const ElfImage &image,
                               ExecuteMode mode,
  ...
  do {
    ...
    if (WIFEXITED(status)) {
      result->last_exit_status = WEXITSTATUS(status);
      // printf("exited, status=%d\n", result->last_exit_status);
    } else if (WIFSIGNALED(status)) {
      result->last_term_signal = WTERMSIG(status);
      // printf("killed by signal %d\n", result->last_term_signal);
    } else if (WIFSTOPPED(status)) {
      ++ptrace_stops_count;
      result->last_stop_signal = WSTOPSIG(status);
      // printf("%4d stopped by signal %d", ptrace_stops_count,
      // result->last_stop_signal);
      ...
      if (ptrace(PTRACE_GETREGS, elf_pid, 0, &regs) == -1) {
        ...
        // call to kill further below. The result fields are set (with the
        // exception of last_term_signal and last_exit_status). This means
        // ReadResultsAndRipOffset was called. Retry ptrace call does not seem to
        // help (as the process seems to be gone). The failure seems innocent, so
        // let's ignore it and just return ptrace_stops_count.
...
void Sandbox::RunElfProcess(const ElfImage &image) const {
  ...
  // (see child_fail) -- never return into Sandbox::Execute.
  //
  ...
  syscall(SYS_execveat, image.fd(), "", (char *const *)av, (char *const *)ep,
          AT_EMPTY_PATH);
  // execveat only returns on failure.
  child_fail("execveat failed");
}

void Sandbox::ReadResultsAndRipOffset(pid_t elf_pid, const ElfImage &image,
                                      unsigned long long rip,
                                      unsigned long start_code,
                                      unsigned long start_data,
                                      ExecutionResult *result) const {
  const SymbolData &symbol_data = image.symbol_data();
  ...
  result->last_rip_offset = (long long)rip - (long long)start_code -
  ...
  if (symbol_data.results_st_size_ %
          sizeof(decltype(result->last_results)::value_type) !=
      0)
    myfail("results_st_size_ mismatch");

  result->last_results.resize(
      symbol_data.results_st_size_ /
      sizeof(decltype(result->last_results)::value_type));
  // printf("last_results size: %ld\n", result->last_results.size());

  local[0].iov_base = result->last_results.data();
