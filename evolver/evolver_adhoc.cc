...
EvolverAdHoc::EvolverAdHoc(int mu, int phi, int lambda, Scorer &scorer,
    ...
    : mu_(mu), phi_(phi), lambda_(lambda),
      programs_(std::vector<std::shared_ptr<Program>>(
          mu_ + lambda_, Program::CreateSimpleSmall())),
      scorer_(scorer), mutator_(mutator), gen_(gen),
      max_generations_(max_generations) {}

// TODO: Put the three stages in individual member functions and add unit tests
// for those.
void EvolverAdHoc::Run() {
  while (current_generation_ < max_generations_) {
    ...
    for (int i = 0; i < lambda_; ++i) {
      // Note: The same program may be selected as both parent1 and parent2 and
      // this is ok for e.g. random recombinations.
